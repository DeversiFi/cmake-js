...
Dist.prototype.ensureDownloaded = async function () {
    if (!this.options.dontDownloadDeps && !this.downloaded) {
        await this.download();
    }
};
...
Dist.prototype._downloadTar = async function (sums) {
    if (this.options.cMakeOptions.CMAKE_JS_INC) return;
...
Dist.prototype._downloadLibs = async function (sums) {
    const log = this.log;
    const self = this;
    if (!environment.isWin || this.options.cMakeOptions.CMAKE_JS_LIB) {
        return;
    }

    const paths = runtimePaths.get(self.targetOptions);
    console.log('Dist.prototype._downloadLibs paths', paths);
